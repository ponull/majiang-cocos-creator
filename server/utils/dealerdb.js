...
function query(sql){
    return new Promise(function(resolve, reject){
        pool.getConnection(function(err,conn){  
            if(err){
                resolve({err:err, rows:null, fields:null});
            }else{  
                conn.query(sql,function(qerr,vals,fields){  
                    //释放连接  
                    conn.release();
                    resolve({err:qerr, rows:vals, fields:fields});
                });  
            }  
        });
    });
...
exports.create_dealer = async function(account,password,name,parent,privi){
    ...
    if(ret.err){
        ...
        await this.update_cumulative(parent,2,1);
        return true;
    }
};
...
/**增加/更新 KPI 数据 */
exports.update_kpi = async function(account,year,month,value)
{
     if(account == null ){
        return false;
    } 
...
exports.get_user_game_info = async function(userid){
    ...
    if(ret.err){
		...
		ret.rows[0].name = Buffer.from(ret.rows[0].name, 'base64').toString();
        return ret.rows[0];
    }
};
...
exports.add_dealer_gems = async function(account,gems){
    ...
    if(ret.err){
        ...
        await this.update_cumulative(account,0,gems);
        return ret.rows.affectedRows > 0;
    }
};
...
exports.add_dealer_score = async function(account,score){
    ...
    if(ret.err){
        ...
        await this.update_cumulative(account,1,score);
        return ret.rows.affectedRows > 0;
    }
};
...
exports.get_goods_all = async function (){
    var nowtime = Date.now;
    var sql = 'SELECT * FROM t_dealers_goods WHERE act_time <= {0} AND ( end_time > {1} OR end_time = -1 ) AND state = 1 ORDER BY act_time DESC';
    sql = sql.format(nowtime,nowtime);
    var ret = await query(sql);
